...
SNRTests = Inf * [1 ]; % state signal-to-noise ratio
SNRCue = Inf;
RLs = [300 ];
...
signalPlotLength = 10;
singValPlotLength = 10;

%%% Setting patterns
patternType = 2; % 1 from intPatternList 
...
set(gcf,'DefaultAxesColorOrder', colWeights'*[1 1 1]);
set(gcf, 'WindowStyle','normal');
set(gcf,'Position', [900 350 400 400]);
...
for p = 1:min(3,NpTestEff)
    ...
    plot(pTemplatesUsedInTest(1:signalPlotLength,p), 'k--', 'LineWidth',2);
    hold off;
    if p < 3
        set(gca, 'XTickLabel',[]);
    end
...
fs = 18; fstext = 18;
set(gcf, 'WindowStyle','normal');
set(gcf,'Position', [1200 100 400 400]);
ColInc = (maxColWeight - minColWeight) / size(RLs,2);
colWeights = minColWeight:ColInc:maxColWeight;
set(gcf,'DefaultAxesColorOrder', colWeights'*[1 1 1]);
hold on;
plot(log10(NRMSEsAligned(:,1)),'s--','LineWidth',3,...
    'Color', colWeights(1)*[1 1 1], 'MarkerSize',12);
plot(log10(NRMSEsAligned(:,2)),'x--','LineWidth',3,...
...
xlabel('Pattern index', 'FontSize', fs);
ylabel('log10 NRMSE', 'FontSize', fs);
