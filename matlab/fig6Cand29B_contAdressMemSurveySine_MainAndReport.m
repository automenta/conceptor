...
N = 200; % set to 100 | 200 for patternType 1 | 2
SR = 1.5;
...
learnWLength = 500; % set to 50 | 500 for patternType 1 | 2
...
SNRTest = 1 ; % state signal-to-noise ratio
SNRCue = Inf;
RL = 10000; % test runlength after cue
...
%%% Setting patterns
patternType = 2; % 1 random same period
...
errorbar(NpLoads, meanNRMSETestOwn, ...
    ...
    'k--', 'LineWidth', 2);
hold off;
set(gca, 'XLim', [1.5 NpLoads(end)+15], 'XTick', NpLoads, ...
    'XScale', 'log', 'FontSize', fs);
xlabel('Nr of loaded patterns', 'FontSize',fstext);
ylabel('log10 NRMSE', 'FontSize',fstext);
%%
% 
% save contentAdressMemSurveyPlotSineMixDataNoisy.mat  ...
%     NpLoads meanNRMSECueOther varNRMSECueOther NSampleOther...
%     meanNRMSETestOther varNRMSETestOther meanNRMSECueOwn ...
%     varNRMSECueOwn  varNRMSECueOwn ...
%     meanNRMSETestOwn varNRMSETestOwn;

%%
figure(3); clf;
  fs = 24; fstext = 24;
set(gcf, 'WindowStyle','normal');
set(gcf,'Position', [800 150  400 400]);

fs = 24; fstext = 24;
hold on;

errorbar(NpLoads(1,[1:4, 6:end]), meanNRMSETestOther(1,[1:4, 6:end]), ...
    1.96*sqrt(varNRMSETestOther(1,[1:4, 6:end])) ./ ...
    sqrt(NSampleOther(1,[1:4, 6:end])), '--',...
    'Color', 0.7*[1 1 1], 'LineWidth', 2);
errorbar(NpLoads(1,[1:4, 6:end]), meanNRMSETestOwn(1,[1:4, 6:end]), ...
    1.96*sqrt(varNRMSETestOwn(1,[1:4, 6:end])) ./ ...
    sqrt(NSampleOwn(1,[1:4, 6:end])), ...
    'k--', 'LineWidth', 2);
hold off;
set(gca, 'XLim', [1.5 NpLoads(end)+15], 'XTick', NpLoads(1,[1:4, 6:end]), ...
    'XScale', 'log', 'YLim',[-1.6 -0.4], 'FontSize', fs);
xlabel('Nr of loaded patterns', 'FontSize',fstext);
ylabel('log10 NRMSE', 'FontSize',fstext);
title('Class Learning Effect','FontSize',fstext);
