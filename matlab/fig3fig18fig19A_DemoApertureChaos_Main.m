...
for p = [1 3 4]
    ...
    apSweepPL = zeros(1, testLength);
        ...
        if p == 4
            plot(apSweepPL(1,1:plotLengthDelayEmbed),apSweepPL(1,1+delay:plotLengthDelayEmbed+delay),'b.');
        else
            plot(apSweepPL(1,1:plotLengthDelayEmbed),apSweepPL(1,1+delay:plotLengthDelayEmbed+delay), 'b','MarkerSize',1);
        end
        rectangle('Position', [0.01, 0.85, 0.45, 0.14],'FaceColor','w',        'EdgeColor','none');
    
        text(0.05,0.93, num2str(alpha,2),'FontSize',18,'FontWeight','bold');
...
for p = 2
    ...
    for i = 1:5
        ...
        text(0.05,0.93, num2str(alphas(i),2),'FontSize',18, ...
            'FontWeight','bold');
...
for p = [2 1 3 4]
    ...
    plot(log10(allAlphas(p,:)), log10(diffPL(p,:)./zsPL(p,:)), 'k', ...
        'LineWidth',2);
    if p == 2
        plot(log10(allAlphasShort(p,:)), yValuesForDots(p) * ones(1,5), 'b.', ...
            'MarkerSize',35);
    else
        plot(log10(allAlphasShort(p,3)), yValuesForDots(p) , 'b.', ...
            'MarkerSize',35);
    end
    hold off;
